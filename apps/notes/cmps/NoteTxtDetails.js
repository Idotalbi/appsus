...
export default {
    ...
    template: `
        ...
        :style="{'background-color': note.bgColor}">
            <input :style="{'background-color': note.bgColor}"  
            type="text" class="note-title-input"
             @input="updateNote" 
             v-model="updatedNote.title" 
             placeholder="Edit Title">
            <textarea :style="{'background-color': note.bgColor}"
             rows="10" 
             cols="20"class="note-txt-input" 
             @input="updateNote" 
             v-model="updatedNote.info.txt" 
