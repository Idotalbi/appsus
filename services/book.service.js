...
function searchGoogleBook(googleBook) {
    return axios.get(`https://www.googleapis.com/books/v1/volumes?q=${googleBook}`)
...
function addBook(book) {
    const newBook = {
        ...
        pageCount: book.volumeInfo.pageCount || 'No page count',
        categories: book.volumeInfo.categories || 'No categories',
        thumbnail: book.volumeInfo.imageLinks.thumbnail || 'No image',
        language: book.volumeInfo.language || 'No language',
        listPrice: {
            amount: 99,
            currencyCode: "EUR",
            isOnSale: false
